...
class QueryResult<E : KIEntity<K>, K : Any>(val q: Query<E, K>, var projections: Map<String, ProjectionResult<E, K>>) {
    ...
    companion object {
        fun <E : KIEntity<K>, K : Any> combine(q: Query<E, K>, results: Iterable<QueryResult<E, K>>): QueryResult<E, K> =
                QueryResult(q, q.projections.map { p -> p.combine(results.map { it.projections[p.name]!! }) }.associateBy { it.name })
