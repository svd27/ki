...
class InheritanceTest : Spek({
    given("") {
        ...
        for (i in 0..9) {
            base.create<MemPerson, String>(MemPersonJvm.Companion.Transient(base.ds, "$i", "${'A' + i}", "${'z' - i}"))
        }
        repeat(10) {
        ...
        on("should the right number of results") {
            ...
            val q = Query<MemPerson, String>(f.cast(), Ordering.NATURAL.cast(), Paging(0, 100))
            val res = runBlocking { base.ds.query(q).getOrElse { throw it }.await() }.getOrElse { throw it }
            it("") {
                res.entites.size `should equal` 20
